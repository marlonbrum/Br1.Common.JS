var Br1Helper =
{
    ...
    addOverlayMsg: function (ctl, message = "Aguarde ...") {
        ...
        msgBg.append(loader);
        msgBg.append($("<span>").text(message));
        ctl[0].overlay_ct = ol;
        $("body").append(ol);
    },
            ...
            searchTextBox.timeoutRef = setTimeout(function () {
                ...
                if (searchValue.length >= minLength) {
                    if (searchTextBox.searchActive) {
                        ...
                        Br1Helper.addOverlayMsg($(searchTextBox).next(), "Pesquisando ...");
    ...
    endSearch: function (inputCtl) {
        ...
        if (next !== null) {
            ...
            Br1Helper.clearOverlay($(inputCtl).next());
    },
    ...
    maskPhone: function (input, dddPadrao) {
        ...
        input.each(function (index, element) {
            ...
            $(element).mask(mascara, optTelefone);
            $(element).attr("maxlenght", 20); 
        });        
    },
    ...
    pesquisaCEP: function (cepInput, parentContainer) {
        cepInput.blur(function (event) {
            ...
            if (cep.length == 8) {
                ...
                $.getJSON(url, null, function (retorno) {
                    if (!retorno.erro)
                    {
    ...
    applyRequiredAttribute: function (event) {
        $("[data-val-required]:not([required])").attr("required", "");
    },
